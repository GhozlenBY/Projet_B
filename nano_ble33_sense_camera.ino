...
/* Includes ---------------------------------------------------------------- */
#include <Training_a_CNN_inferencing.h>
...
*/
const float seuil_de_classification = 0.5;
const int numCategories = 5;
int componentCounts[numCategories] = {0};

void setup() {
    // ... [Configuration initiale du code existant]

    // Initialisation des compteurs de composants
...
for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
        if (result.classification[ix].value > seuil_de_classification) { // Remplacer par le seuil approprié
            componentCounts[ix]++;
        }
    }

    // Afficher les compteurs pour chaque catégorie de composants
    for (int i = 0; i < numCategories; i++) {
        ei_printf("Catégorie %d: %d\n", i, componentCounts[i]);
    }
